	...
	const onLoginEnhance = ({ cancel }) => {
		...
		return async ({ result, update }) => {
			...
			if (result.type === 'success') {
				...
				var emailError = result.data?.details?.email;
				var passwordError = result.data?.details?.password;
...
<div class="w-full max-w-md">
	<div class=" text-center">
		<h1 class="mb-4 text-4xl font-bold">Log In</h1>
	...
	<form
		method="POST"
		action="/login"
		use:enhance={onLoginEnhance}
		class="card mx-auto mt-2 card-body max-w-sm"
		novalidate
	>
		<div>
		...
		<button type="submit" class="btn mt-2 w-full text-primary-content btn-lg btn-primary"
			>Continue</button
		>
	</form>
